	...
	dependencyTypes.forEach((depType) => {
		...
		if (depsOfType) {
			Object.keys(depsOfType).forEach((depName) => {
				...
				const installData =
					installedModules.find(moduleFilter) ||
					rootDependencies.find(moduleFilter);

				const targetVersion = depsOfType[depName];
...
async function getPackageInfo(packageData) {
	const { targetVersion, installedVersion, apps, name, type } = packageData;

	try {
		const registryData = await pacote.packument(name, {}); // TODO: add optional options?
		const versions = Object.keys(registryData.versions);

		const wanted = semver.maxSatisfying(versions, targetVersion);
		// TODO: support more semver types
		const wildcards = ['^', '~'];
		const currentWildcard =
			wildcards.find((wildcard) => targetVersion.includes(wildcard)) ||
			'';
		...
		const installNeeded =
			missing ||
			(targetVersion === wantedRange &&
				installedVersion !== wantedVersion);

		// upgrades
		const upgradableToWanted = targetVersion !== wantedRange;
		const upgradableToLatest = targetVersion !== latestRange;
...
async function getPackages({
	...
	*/
	...
	const packageLocations = [rootPath];
	...
	if (monorepo) {
		...
		if (hoisted) {
			...
			} = await getPackageJsonAndModules(
				rootPath,
				(node) => !node.parent
			);
