...
async function getPackageInfo({
	...
	try {
		...
		const registryData = await pacote.packument(name, { fullMetadata: true }); // TODO: add optional options?
		...
		const npmsInfo = await (await fetch(`https://api.npms.io/v2/package/${name}`)).json();
...
function processDependencies(dependencies, updateProgress, { concurrency = 8 } = {}) {
	/*
...
async function getPackages(
	...
	*/
	...
	dependencies = await processDependencies(dependencies, updateProgress, { concurency: 8 })
