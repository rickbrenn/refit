...
function readFileData(directory, file) {
	try {
...
function readPackageFiles(config, directory) {
	try {
		const packageJsonData = readFileData(directory.path, 'package.json');

		const packageLockData = readFileData(
			directory.path,
			'package-lock.json'
		);
				...
				return [
					...
					...Object.keys(packageJsonData[type]).map((packageName) => {
						return {
							...
							current:
								packageLockData?.dependencies[packageName]
									?.version,
						};
					}),
				];
...
async function getPackageInfo(packageData) {
	...
	try {
		...
		const currentWildcard =
			wildcards.find((wildcard) => target.includes(wildcard)) || '';

		// versions
		const installedVersion = null; // TODO: do something with this
